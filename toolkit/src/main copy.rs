...
fn command_ping_test(list_ip: Vec<String>, username: String, password: String) -> Vec<String> {
    ...
    while i < j {
        ...
        if i < j {
            let cmd2 = format!("wt.exe -w 10 sp --title {} -V -c {} {} {} {} {} {} {}",
                titletab[n], api, list_ip[i], list_ip[i+1], list_ip[i+2], list_ip[i+3], username, password);
            i += 4;
            commands.push(cmd2);
        } else {
        ...
        commands.push("wt.exe -w 10 mf left".to_string());

        if i < j {
            let cmd4 = format!("wt.exe -w 10 sp --title {} -H -c {} {} {} {} {} {} {}",
                titletab[n], api, list_ip[i], list_ip[i+1], list_ip[i+2], list_ip[i+3], username, password);
            i += 4;
            commands.push(cmd4);
        } else {
        ...
        commands.push("wt.exe -w 10 mf right".to_string());

        if i < j {
            let cmd6 = format!("wt.exe -w 10 sp --title {} -H -c {} {} {} {} {} {} {}",
                titletab[n], api, list_ip[i], list_ip[i+1], list_ip[i+2], list_ip[i+3], username, password);
            i += 4;
            n += 1;
            commands.push(cmd6);

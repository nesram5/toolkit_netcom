...
fn start() ->  io::Result<()> {
    ...
    if args.len() < 8 {
        eprintln!("Usage: {} <title> <host> <username> <password> <source_address> <destination_address> <on_live_test_or_report>", args[0]);
        std::process::exit(1);
    }
    let port: &str = "22";
    ...
    1 =>{
       ...
       ping_test_mode(&username, &password,address, title.to_string(), command);
    }    
    2 => {
        let command = format!("ping {} src-address={}", destination_address, source_address);
